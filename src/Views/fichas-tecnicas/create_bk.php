            ...
            <div class="mt-6 border-t pt-4">
                ...
                <div class="flex justify-between items-center mb-2">
                    <div class="flex items-center gap-2">
                        <!-- BOTÓN VOLVER (como pediste siempre presente) -->
                    ...
                    <button type="button" id="addProceso"
                        class="text-xs bg-blue-600 text-white px-2 py-1 rounded">
                        + Añadir proceso
                    </button>
                </div>

                <div id="procesos_ft" class="space-y-2">

                    <div class="flex gap-2 bg-gray-50 p-2 rounded border">

                        <div class="flex gap-2 bg-gray-50 p-2 rounded border">

                            <select name="procesos_ft[0][codigo_proceso]"
                                class="js-example-basic-single w-56"
                                required>
                                <option value="">Seleccione proceso</option>
                                <?php foreach ($procesosCatalogo as $proc): ?>
                            ...
                            <input type="hidden"
                                name="procesos_ft[0][nombre_proceso]"
                                class="nombre_proceso_hidden">

                            <input type="number"
                                name="procesos_ft[0][orden]"
                                placeholder="Orden"
                                class="w-20 border p-1 rounded">

                            <select name="procesos_ft[0][ejecutable_en]"
                                class="w-32 border p-1 rounded">
                            ...
                            <input type="text"
                                name="procesos_ft[0][comentario]"
                                placeholder="Comentario"
                                class="w-48 border p-1 rounded">

                            <label class="text-xs flex items-center gap-1">
                                <input type="checkbox" name="procesos_ft[0][requiere_diseno]">
                                Diseño
                            </label>

                            <label class="text-xs flex items-center gap-1">
                                <input type="checkbox" name="procesos_ft[0][requiere_molde]">
                                Molde
                            </label>
...
<script>
    ...
    $('#addProceso').click(function() {
        ...
        const nuevoID = `proc_select_${procIndex}`;

        const html = `
    <div class="flex gap-2 bg-gray-50 p-2 rounded border mt-2">

        <select name="procesos_ft[${procIndex}][codigo_proceso]"
                id="${nuevoID}"
                class="js-example-basic-single w-56"
                required>
            <option value="">Seleccione proceso</option>
        ...
        <input type="hidden"
               name="procesos_ft[${procIndex}][nombre_proceso]"
               class="nombre_proceso_hidden">

        <input type="number"
               name="procesos_ft[${procIndex}][orden]"
               placeholder="Orden"
               class="w-20 border p-1 rounded">

        <select name="procesos_ft[${procIndex}][ejecutable_en]"
                class="w-32 border p-1 rounded">
            <option value="AMBOS">Ambos</option>
            <option value="PLANTA">Planta</option>
            <option value="SATELITE">Satélite</option>
        ...
        <input type="text"
               name="procesos_ft[${procIndex}][comentario]"
               placeholder="Comentario"
               class="w-48 border p-1 rounded">

        <label class="text-xs flex items-center gap-1">
            <input type="checkbox" name="procesos_ft[${procIndex}][requiere_diseno]">
            Diseño
        </label>

        <label class="text-xs flex items-center gap-1">
            <input type="checkbox" name="procesos_ft[${procIndex}][requiere_molde]">
            Molde
        </label>

        <button type="button" class="remove-proc text-red-600 px-2">X</button>

    </div>
    `;

        $('#procesos_ft').append(html);

        aplicarSelect2(`#${nuevoID}`); // 👈 importante

        procIndex++;
    });
    $(document).on('change', 'select[name*="[codigo_proceso]"]', function() {
        const texto = $(this).find("option:selected").text();
        $(this).closest('div').find('.nombre_proceso_hidden').val(texto);
    });
</script>